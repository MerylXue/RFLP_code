...
def run_Wass_eps(rawdata_train_lst, rawdata_test_lst, info, num_node, num_data, num_cov, beta_, eps):
    sim_data_driven_cost = np.zeros(NumSimulate)
    sim_out_of_sample_saa = np.zeros(NumSimulate)
    reliability = 0.0
    time_sol = 0.0
    avg_gap = 0.0
    outputfile_wasser = open('result/Reliability/Wasserstein_%d_%d_%f_Eps%f.csv' % (
        num_data, num_node, beta_,eps), 'w')
    ...
    for l in range(NumSimulate):
        ...
        time_sol += t_wasser
        avg_gap  += gap_wasser
        saa_wasser = SAACost(loc_wasser, info['fixed_cost'], info['dist'], num_node, rawdata_test)
    ...
    outputfile = open('result/Reliability_Wasserstein_Node%d_Data%d_Cov%d_Beta%.4f_Eps%f.csv' % (num_node, num_data, num_cov, beta_,eps), 'w')
...
def test_Reliability_Wasserstein(rawdata_train_lst, rawdata_test, info, num_node, num_data, num_cov, beta_):
    ...
    outputfile_wasser = open('result/Reliability/Wasserstein_data%d_node%d_cov%d_beta%f.csv' % (
             num_data, num_node, num_cov, beta_), 'w')
    ...
    ## output reliability result
    outputfile = open(
        'result/Reliability_Wasserstein_Node%d_Data%d_Cov%d_Beta%.4f.csv' % (num_node, num_data, num_cov, beta_),
        'w')
...
def run_Storm_Wasserstein(train_data_lst, test_data_lst, info, num_node, num_cov, beta_,  train_length, test_length):
    outputfile = open(
        'result/Storm/Wasserstein-Date_%s-Node_%d-Cov_%d-Beta_%.2f.csv' % (date.today().strftime("%Y_%m_%d"), num_node, num_cov,  beta_),
        'w')
