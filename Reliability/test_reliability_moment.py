...
def test_Reliability_mm(rawdata_train_lst, rawdata_test, info, num_node, num_data, num_cov):
    n_method = 4
    ## remark the position of each method in the output file
    idx_cm_cov = 0
    idx_mm_cov = 1
    # sim_data_driven_cost = [0 for i in range(n_method)]
    ...
    # sim_out_of_sample_saa = [0 for i in range(n_method)]
    ...
    for i in range(n_method):
        # Opt_EpsSet[i] = Opt_EpsSet[i]/NumSimulate
        # Opt_EpsSamplesSet[i] = [Opt_EpsSamplesSet[i][k]/NumSimulate for k in range(len(Opt_EpsSamplesSet[i]))]
        # sim_data_driven_cost[i] = sim_data_driven_cost[i] / NumSimulate
        ...
        # in_sample_rel[i] = in_sample_rel[i] / NumSimulate
        ...
        # count_search[i] = count_search[i]/ NumSimulate
        # time_search[i] = time_search[i] /NumSimulate
...
def test_Reliability_mm_NoCov(rawdata_train_lst, rawdata_test, info, num_node, num_data, num_cov, mu_coeff, truncate):
    n_method = 2
    ## remark the position of each method in the output file
    # idx_cm_cov = 0
    idx_cm_novov = 0
    ...
    outputfile_mmn = open(
        'result/Reliability/CrossMoment_nocov_data%d_node%d_cov_%d_Mu_%f_Truncate_%d.csv' % (
            num_data, num_node, num_cov, mu_coeff, truncate), 'w')

    # outputfile_lvc = open('result/Reliability/MarginalMoment_cov_data%d_node%d_cov%d.csv' % (
    #     num_data, num_node, num_cov), 'w')
    outputfile_lvn = open('result/Reliability/MarginalMoment_nocov_data%d_node%d_cov%d_Mu_%f_Truncate_%d.csv' % (
             num_data, num_node, num_cov, mu_coeff, truncate), 'w')
    outline1 = ['index', 'in_sample_cost', 'out_of_sample_cost', 'Str_loc', 'sol_time', 'gap']
    ...
    ## output reliability result
    outputfile = open(
        'result/Reliability_moment_Node%d_Data%d_Cov%d_Mu_%f_Truncate_%d.csv' % (num_node, num_data, num_cov, mu_coeff, truncate),
        'w')
    out_line_col0 = ['num_data', 'Method', 'in_sample_cost_avg', 'in_sample_cost_q1',
                     'in_sample_cost_q2', 'in_sample_cost_q8', 'in_sample_cost_q9',
                     'out_of_sample_cost_avg',
...
def run_Storm_mm(train_data_lst, test_data_lst, info, num_node, num_cov, beta_, train_length, test_length):
    outputfile = open(
        'result/Storm/Moment-Node_%d-Cov_%d-Beta_%.2f.csv' % ( num_node, num_cov, beta_),
        'w')
